...
def descargar_video(url, resolucion, carpeta="Descargas"):
    """
...
if __name__ == "__main__":
    ...
    resoluciones_disponibless = obtener_resoluciones_disponibles(video_url)
    if not resoluciones_disponibless:
        print("No se encontraron resoluciones disponibles para este video.")
    else:
        print(f"Resoluciones disponibles: {', '.join(resoluciones_disponibless)}")
    video_resolucion = input("Elige la resolución: ").lower()
    if video_resolucion in resoluciones_disponibless:
        descargar_video(video_url, video_resolucion)
    else:
        print("Resolución no válida. Elija una de las resoluciones disponibles.")
